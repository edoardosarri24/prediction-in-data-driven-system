#!/usr/bin/env python3
import re
from collections import defaultdict

### relative path ###
base_bias_path = 'base_bias.pl'
bias_path = 'bias.pl'
log_path = 'trace.log'
bk_path = 'bk.pl'
exs_path = 'exs.pl'

### functions ###
def augment_bias():
    # element
    bias_elements = []
    tasks_chunks = task_chunk_dictionary()
    # generate new bias
    try:
        with open(base_bias_path, 'r') as file:
            max_chunk=0
            this_chunk=0
            for line in file:
                line = line.strip()
                # release line
                if re.search(r'\brelease_task\b', line):
                    for task in sorted(list(tasks_chunks.keys())):
                        new_line = line.replace('release_task', f'release_task{task}')
                        bias_elements.append(new_line)
                # task line
                elif re.search(r'\btask\b', line):
                    for task in sorted(list(tasks_chunks.keys())):
                        new_line = re.sub(r"\btask\b", f"task{task}", line)
                        bias_elements.append(new_line)
                # chunk line
                elif re.search(r'\bchunk\b', line):
                    for task in sorted(list(tasks_chunks.keys())):
                        this_chunk = len(list(tasks_chunks[task]))
                        if (this_chunk > max_chunk):
                            max_chunk = this_chunk
                    for chunk in range (max_chunk):
                        new_line = re.sub(r"\bchunk\b", f"chunk{chunk}", line)
                        bias_elements.append(new_line)
                # complete line
                elif 'complete' in line:
                    for task in sorted(list(tasks_chunks.keys())):
                        new_line = line.replace('complete_task', f'complete_task{task}')
                        bias_elements.append(new_line)
                # execute line
                elif 'execute' in line:
                    for task in sorted(list(tasks_chunks.keys())):
                        for chunk in sorted(list(tasks_chunks[task])):
                            new_line = line.replace('execute_task_chunk', f'execute_task{task}_chunk{chunk}')
                            bias_elements.append(new_line)
                # finish line
                elif 'finish' in line:
                    for task in sorted(list(tasks_chunks.keys())):
                        for chunk in sorted(list(tasks_chunks[task])):
                            new_line = line.replace('finish_task_chunk', f'finish_task{task}_chunk{chunk}')
                            bias_elements.append(new_line)
                # executionTime line
                elif 'executionTime' in line:
                    for task in sorted(list(tasks_chunks.keys())):
                        for chunk in sorted(list(tasks_chunks[task])):
                            new_line = line.replace('executionTime_task_chunk', f'executionTime_task{task}_chunk{chunk}')
                            bias_elements.append(new_line)
                # other lines
                else:
                    bias_elements.append(line)
    except FileNotFoundError:
        print(f"Error: {base_bias_path} not found.")
        return
    except Exception as e:
        print(f"An error occurred during file copy: {e}")
        return
    # Write the output files
    with open(bias_path, 'w') as output:
        for bias_element in bias_elements:
            output.write(f"{bias_element}\n")
    print('bias.pl sucessfully generated!!!')

def generate_bk_exs():
    # element
    bk_elements = []
    bk_elements.append(':-style_check(-discontiguous). % dont considere the discountinuity of clauses\n')
    exs_elements = []
    exs_elements.append(':-style_check(-discontiguous). % dont considere the discountinuity of clauses\n')
    trace_id = 0
    executionTimes = {}
    release_number = defaultdict(int)
    try:
        with open(log_path, 'r') as file:
            for line in file:
                line = line.strip()
                # skip blank line
                if not line:
                    continue
                # end line
                if 'end' in line:
                    bk_elements.append('')
                    exs_elements.append(f'neg(failure(trace_{trace_id})).')
                    trace_id += 1
                    release_number.clear()
                    continue
                # deadlineMiss line
                if 'deadlineMiss' in line:
                    bk_elements.append('')
                    exs_elements.append(f'pos(failure(trace_{trace_id})).')
                    trace_id += 1
                    release_number.clear()
                    continue
                # release line
                if 'release' in line:
                    match = re.search(r'<(\d+\.\d+).*?Task(\d+)', line)
                    if match:
                        time = match.group(1)
                        time = int(float(time)*1000) # time in microseconds
                        task_id = match.group(2)
                        release_number[task_id] += 1
                        bk_element = (f'task{task_id}(trace{trace_id},releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}).')
                        bk_elements.append(bk_element)
                        bk_element = (f'release_task{task_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}, {time}).')
                        bk_elements.append(bk_element)
                # complete line
                if 'complete' in line:
                    match = re.search(r'<(\d+\.\d+).*?Task(\d+)', line)
                    if match:
                        time = match.group(1)
                        time = int(float(time)*1000) # time in microseconds
                        task_id = match.group(2)
                        bk_element = (f'complete_task{task_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}, {time}).')
                        bk_elements.append(bk_element)
                # execute line
                if 'execute' in line:
                    match = re.search(r'<(\d+\.\d+).*?Chunk(\d+).(\d+)', line)
                    if match:
                        time = match.group(1)
                        time = int(float(time)*1000) # time in microseconds
                        task_id = match.group(2)
                        chunk_id = match.group(3)
                        bk_element = (f'chunk{chunk_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}, releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}_chunk{chunk_id}).')
                        bk_elements.append(bk_element)
                        bk_element = (f'execute_task{task_id}_chunk{chunk_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}_chunk{chunk_id}, {time}).')
                        bk_elements.append(bk_element)
                        executionTimes[(trace_id, task_id, chunk_id)] = time
                # finish line
                if 'finish' in line:
                    match = re.search(r'<(\d+\.\d+).*?Chunk(\d+).(\d+)', line)
                    if match:
                        time = match.group(1)
                        time = int(float(time)*1000) # time in microseconds
                        task_id = match.group(2)
                        chunk_id = match.group(3)
                        bk_element = (f'finish_task{task_id}_chunk{chunk_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}_chunk{chunk_id}, {time}).')
                        bk_elements.append(bk_element)
                        executionTime = time - executionTimes[(trace_id, task_id, chunk_id)]
                        bk_element = (f"executionTime_task{task_id}_chunk{chunk_id}(releaseNumber{release_number[task_id]}_trace{trace_id}_task{task_id}_chunk{chunk_id}, {executionTime}).")
                        bk_elements.append(bk_element)
    # exception handling
    except FileNotFoundError:
        print(f"Error: Input file not found at {log_path}")
        return
    except Exception as e:
        print(f"An error occurred: {e}")
        return
    # Write the output files
    with open(bk_path, 'w') as output:
        for bk_element in bk_elements:
            output.write(f"{bk_element}\n")
    print('bk.pl sucessfully generated!!!')
    with open(exs_path, 'w') as output:
        for exs_element in exs_elements:
            output.write(f"{exs_element}\n")
    print('exs.pl sucessfully generated!!!')

### helper ###
def task_chunk_dictionary():
    task_chunks = {}
    try:
        with open(log_path, 'r') as file:
            for line in file:
                line = line.strip()
                # key (tasks) initialization
                if 'release' in line:
                    match = re.search(r'Task(\d+)', line)
                    if match:
                        task_id = int(match.group(1))
                        if task_id not in task_chunks:
                            task_chunks[task_id] = set()
                # value (chunks for task) appending
                elif 'execute' in line:
                    match = re.search(r'Chunk(\d+).(\d+)', line)
                    if match:
                        task_id = int(match.group(1))
                        chunk_id = int(match.group(2))
                        task_chunks[task_id].add(chunk_id)
    except FileNotFoundError:
        print(f"Error: {log_path} not found.")
        return None
    except Exception as e:
        print(f"An error occurred: {e}")
        return None
    return task_chunks

### main ###
if __name__ == '__main__':
    augment_bias()
    generate_bk_exs()