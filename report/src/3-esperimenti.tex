...
Per implementare questo è stato modificato il representation language in modo che da forzare il sistema a non mettere in relazione tracce diverse nel corpo della regola con quella definita nella testa e ridurre quindi lo spazio di ricerca. Per andare più nel dettaglio si guardi il \href{https://github.com/edoardosarri24/prediction-in-data-driven-system/blob/main/my-experiments/13-add-in/bias.pl}{bias.pl} dell'esperimento in questione. Con questa idea la regola che cerchiamo ha la forma di \texttt{failures(A) :- task1(A,B), release\_task1(B,C), chunk2(B,D), chunk3(B,E), executionTime\_task1\_chunk2(D,F), \\ executionTime\_task1\_chunk3(E,G), add(F,G,H), ge(H,500).}: in pratica si è cercato di legare il fallimento prima a un task poi a un chunk e di legare il tempo di esecuzione a questi due elementi; in questo modo il sistema ha meno libertà per comporre la regola; in contrasto il numero di letterali nella regla che si cerca uamenta.

Il risultato di questo esperimento però non è stato soddisfacente. È stato usato un timeout di 172800 secondi (i.e., 48 ore); il sistema continuava ad eseguire con un utilizzo della CPU del 99\%, ma senza provare regole nuove. Come si vede dal file \href{https://github.com/edoardosarri24/prediction-in-data-driven-system/blob/main/my-experiments/13-add-in/result.txt}{result.txt} alle 15 del pomeriggio l'ultima regola aggiornata era delle 9 di mattina; il motivo potrebbe essere dovuto a una ricorsione troppo profonda in cui il sistema si è trovato e che non gli permette di esplorare nuove regole.