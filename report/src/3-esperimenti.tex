...
Ragionando sul representation bias il problema potevare essere dovuto al fatto che la somma dei due campionamenti non solo doveva essere maggiore di 0.5ms, ma doveva esserlo anche nello stesso rilascio del Task1: se il Chunk1.2 campiona un valore molto alto e il Chunk1.3 un valore molto basso, dove la somma è minore di 0.5ms e in un rilascio successivo avviene il viceversa, allora in questo caso non si ha fallimento; se invece Chunk1.2 e Chunk1.3 campionano consucutiamente du tempi la cui somma è maggiore di 0.5ms, allora la traccia fallisce perché il Task2 non ha tempoi di esegurie.

Il representation bias fino a questo momento era esattamente quello definito per gli esperimenti precedenti. Per aumentare il potere descrittivo del sistema e cercare che a

Per includere l'informazione della release del task è stata aumentata l'arietà dei predicati in modo che venga incluso anche un id incrementale (che si resetta all'inizio di ogni traccia) che indica il numero di rilascio.

\subsection{Risultati}
Nonostante aver aumentato la Descrittività dei predicati