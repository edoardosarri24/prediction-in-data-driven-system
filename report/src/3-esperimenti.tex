...
Per implementare questo è stato modificato il representation language in modo che da forzare il sistema a non mettere in relazione tracce diverse nel corpo della regola con quella definita nella testa e ridurre quindi lo spazio di ricerca. Per andare più nel dettaglio guardare il \href{https://github.com/edoardosarri24/prediction-in-data-driven-system/blob/main/my-experiments/13-add/bias.pl}{bias.pl} dell'esperimento in questione. Con questa idea la regola che cerchiamo ha la forma di \texttt{failure(A):- task(A,B), executionTime\_1\_2(B,C), executionTime\_1\_3(B,D), add(C,D,E), geq(E,501)}, cioè una regola che ha 6 letterali.

Con questa idea il sistema effettivamente diminuisce lo spazio di ricerca: il timeout scatta ancora, però mentre sta cercando regole di lunghezza 6, che è proprio la lunghezza della regola che stiamo cercando. Si nota ancora comunque che ci sono delle regole dove compare \texttt{failure(A)} e \texttt{Task(x;C)} con \texttt{x$\ne$A}.
